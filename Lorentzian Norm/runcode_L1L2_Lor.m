...
for h = 1 : lensize
    ...
    for repeat = 1 : repeats
        ...
        noise = 0.005*tan(pi*(rand(m, 1) - 1/2));  % Cauchy noise
        b = A*x0 + noise;
       ...
       fprintf('Start of SPGL1 \n');
        ...
        tstart2 = tic;
        x_spgl1 = spg_bpdn(A1, b1, sigma1, optsL1);
        t_spgl1 = toc(tstart2);
        
        fval_spgl1 = norm(x_spgl1, 1) - mu*norm(x_spgl1);
        ...
        tfeas = tic;
        M = (norm(xslater, 1) - mu*norm(xslater))/(1 - mu); % Upper bound of x
        ...
        % SQP
        tsqp = tic;
        [x_sqp, iter_sqp, flag_sqp] = L1L2_Lor_SQP_retract(A, b, gamma, sigma, mu, xin, xslater, L, M, maxiter, freq, tol);
        t_sqp = toc(tsqp);
        
        if flag_sqp ==1
            iter_sqp_s = iter_sqp_s +1;
        end
        fval_sqp = norm(x_sqp, 1) - mu*norm(x_sqp);
        ...
        % ESQM with delta1
        tesqm1 = tic;
        [x_esqm1, iter_esqm1, flag_esqm1] = L1L2_Lor_ESQM_ls(A, b, gamma, sigma, mu, xin, delta1, L, M, maxiter, freq, tol);
        t_esqm1 = toc(tesqm1);
        
        if flag_esqm1 == 1
            iter_esqm_s1 = iter_esqm_s1 +1;
        end
        fval_esqm1 = norm(x_esqm1, 1) - mu*norm(x_esqm1);
        ...
        % ESQM with delta2
        tesqm2 = tic;
        [x_esqm2, iter_esqm2, flag_esqm2] = L1L2_Lor_ESQM_ls(A, b, gamma, sigma, mu, xin, delta2, L, M, maxiter, freq, tol);
        t_esqm2 = toc(tesqm2);
        
        if flag_esqm2 == 1
            iter_esqm_s2 = iter_esqm_s2 +1;
        end
        fval_esqm2 = norm(x_esqm2, 1) - mu*norm(x_esqm2);
        ...
        % ESQM with delta3
        tesqm3 = tic;
        [x_esqm3, iter_esqm3, flag_esqm3] = L1L2_Lor_ESQM_ls(A, b, gamma, sigma, mu, xin, delta3, L, M, maxiter, freq, tol);
        t_esqm3 = toc(tesqm3);
        
        if flag_esqm3 == 1
            iter_esqm_s3 = iter_esqm_s3 +1;
        end
        fval_esqm3 = norm(x_esqm3, 1) - mu*norm(x_esqm3);
...
a = clock;
fname = ['Results\LorNor_table' '-'  date '-' int2str(a(4)) '-' int2str(a(5)) '.txt'];
fid = fopen(fname, 'w');
