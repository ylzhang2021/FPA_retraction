...
it is natural to expect SPGL1 to return a solution in the interior of the feasible set of the original problem, unfortunately we cannot mathematically guarantee this. %For example, for those instances in Section 6.2, the problem that SPGL1 solves in this case is (6.8), whose constraint set is contained \emph{inside} the feasible set of the original problem. 
Therefore, we prefer
not to mention this. 
