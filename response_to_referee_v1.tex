...
\begin{enumerate}
	...
	\textbf{Response:} Thanks for the comment. This can be see from [9, p341]. We have modified it accordingly.
...
which is a quadratic function in $\tau$. It is easy to calculate $\tau$.
	...
	\textbf{Response:} Thanks for the comment. We choose different initial point for comparison in problem (6.1). When the initial point
$x^0 = (0, 0, \cdots, 0)$, we have the results in Table 1; When the initial point $x^0$ is given as in (6.6), we have the results in Table 2. Since the problem is non-convex, we can see from the results that the initial value has great influence on the efficiency of the algorithms. Although ESQM$_{\rm ls}$ is faster than FPA$_{\rm retract}$ when the initial value is zero, FPA$_{\rm retract}$ is faster than ESQM$_{\rm ls}$ when the initial point $x^0$ is given as in (6.6). 

\begin{table}[H]
\caption{Computational results for problem (6.1) with initial point $x^0 = (0, 0, \cdots, 0)$}\label{table1}
\begin{center}
{\footnotesize
...
\begin{table}[H]
\caption{Computational results for problem (6.1) with initial point $x^0$ is given as in (6.6)}\label{table1}
\begin{center}
{\footnotesize
\begin{tabular}{|c|c|c|c|c|c|c|}\hline
...
Indeed, the above seems a better title for the work because it connects the core ideas in the manuscript.}
    ...
    \textbf{Response:} Thanks for the comment. We compare the algorithms with SCP presented in [42] on solving problem (6.3):
    \begin{equation}\label{pr1}
       ...
       \end{array}
     \end{equation}
    Then, we have the results in Table 3.
...
From Table 3, one can see that the recovery errors of the algorithms are comparable, and SCP is usually faster. Since SCP can take advantage of BB step size, but our algorithm cannot use it.

Recently, some scholars consider compressed sensing with Cauchy noise in complex number, which can be translated into the following problem:
