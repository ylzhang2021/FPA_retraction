...
% Parameter settings
indexsize = 3;% [2, 4,  6, 8, 10];%5;% % problem size
repeats = 3;
maxiter = inf;
...
for h = 1 : lensize
    ...
    m = 720*index;
    n = 2560*index;
    J = 5;  % size of each block
    k = 60*index;% nonzero blocks
   ...
   iter_esqm_s3 = 0;
    ...
    for repeat = 1 : repeats
        ...
        % ESQM with delta1
        tesqm1 = tic;
        [x_esqm1, iter_esqm1, flag_esqm1] = GL_ESQM_ls(A, b, sigma, mu, J, xin, delta1, L, M, maxiter, freq, tol);
        t_esqm1 = toc(tesqm1);
        ...
        % ESQM with delta2
        tesqm2 = tic;
        [x_esqm2, iter_esqm2, flag_esqm2] = GL_ESQM_ls(A, b, sigma, mu, J, xin, delta2, L, M, maxiter, freq, tol);
        t_esqm2 = toc(tesqm2);
        ...
        % ESQM with delta3
        tesqm3 = tic;
        [x_esqm3, iter_esqm3, flag_esqm3] = GL_ESQM_ls(A, b, sigma, mu, J, xin, delta3, L, M, maxiter, freq, tol);
        t_esqm3 = toc(tesqm3);
...
a = clock;
fname = ['Results\GL_table' '-'  date '-' int2str(a(4)) '-' int2str(a(5)) '.txt'];
fid = fopen(fname, 'w');
